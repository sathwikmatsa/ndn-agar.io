...
int main() {
  ...
  egim.player_index = 9;
  egim.players =
      std::vector<std::tuple<std::string, uint8_t, uint8_t, uint8_t>>(
          2, std::make_tuple("Dhruva", r, g, b));
  egim.pellets = std::vector<std::tuple<int, int, uint8_t, uint8_t, uint8_t>>(
      2, std::make_tuple(x, y, r, g, b));
  ...
  PlayerStats einfo;
  einfo.cells = std::vector<std::tuple<float, float, float>>(
      2, std::make_tuple(10.5, 20.5, 30.4));
  einfo.ejectiles =
      std::vector<std::tuple<float, float>>(2, std::make_tuple(4.0, 9.9));
  ...
  esm.stats =
      std::vector<PlayerStats>(2, einfo); // Reuse initialised PlayerStats

  // decode
  stream.is_writing = false;
  SnapshotMessage dsm;
  dsm.serialize(stream);
  assert(esm.seq_id == dsm.seq_id);

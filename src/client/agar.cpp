...
void Agar::follow_mouse(int mx, int my, Camera &camera) {
    ...
    std::sort(cells.begin(), cells.end(), [=](Cell &m, Cell &n) {
        float dist_m =
            std::sqrt(std::pow(mouse_x - (m.x - camera_x_offset), 2) +
                      std::pow(mouse_y - (m.y - camera_y_offset), 2));
        float dist_n =
            std::sqrt(std::pow(mouse_x - (n.x - camera_x_offset), 2) +
    ...
    for (int i = 0; i < n_cells; i++) {
        ...
        float speed_factor =
            std::max(MAX_SPEED + (MIN_SPEED - MAX_SPEED) /
                                     (MAX_AGAR_RADIUS - AGAR_RADIUS) *
